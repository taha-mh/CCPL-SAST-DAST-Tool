    ...
    for index, finding in enumerate(target_findings, start=1):
        ...
        if assessment.get("llm_status") == "error":
            ...
            finding["llm_review"] = {
                ...
                "confidence": "LOW",
                "llm_status": "error",
                "error_type": "assessor_failure",
            }
        else:
            user_prompt = build_reviewer_user_prompt(finding)
            review_verdict = query_llm(
                REVIEWER_SYSTEM_PROMPT,
                user_prompt,
            ...
            if review_verdict.get("llm_status") == "error":
                finding["llm_review"] = {
                    **review_verdict,
                    "decision": "needs_review",
                    "review_reason": "The reviewer API did not produce a valid verdict; manual verification is required.",
                    "final_severity": assessment.get("severity", finding.get("scanner_severity", "LOW")),
                    "confidence": "LOW",
                }
            else:
